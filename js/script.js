...
jQuery(document).ready(function() {
	...
	jQuery(document).on('click', '.close, .black-bg, [data-close]', function (e) {
		...
		if(message !== undefined) {
			...
			//jQuery('.select-overlay.active').removeClass('active').remove();
		} else {
	...
	jQuery(".filter-price__input .price").inputmask("currency",{ "placeholder": " ",
        groupSeparator: ' ',
        autoGroup: true,
		suffix: ' ₽',
		digits: 0,
	...
	jQuery(".filter-price__input .number").inputmask("decimal",{ "placeholder": " ",
        groupSeparator: ' ', // Optional: define group separator as well
        autoGroup: true,
        digits: 0, // Number of digits after the radix point
        digitsOptional: false,
		radixPoint: '',
    });
